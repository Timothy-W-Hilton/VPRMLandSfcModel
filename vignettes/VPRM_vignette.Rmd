...
par_est_status <-
    estimate_VPRM_pars(all_data=pfa_dd[['data']],
                       DE_itermax = 2,
                       par_set_str='Example_PFa_Parameters_Monthly',
...
optimized parameters are a list of objects of class DEoptim.  See the
documentation for the [DEoptim package](https://cran.r-project.org/package=DEoptim) for details.  The best-fit
values are in the field optim$bestmem.  VPRMLandSurfaceModel includes
a dataset containing the output of the monthly parameter estimation in
the above example.  See below for a usage example.

# Usage Example: using previously estimated VPRM parameters #

```{r, fig.show='hold'}
library(VPRMLandSfcModel)
## loads the parameter values from the above example
data(Example_PFa_Parameters_Monthly)
print(names(Example_PFa_Parameters_Monthly))
print(Example_PFa_Parameters_Monthly[['Jul.2005']][['optim']][['bestmem']])
